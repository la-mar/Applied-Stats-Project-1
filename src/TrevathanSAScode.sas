proc import datafile="/home/mtrevathan0/train.csv"  /*I have a similar import line but the missing=0 line makes sense*/
...
/* Original code attempt. Checking the data for variable types. All character variables
are listed in "Char variable list.txt" in the "TrevathanEDA" folder of "src."*/
proc contents data=train; run;

/* An attempt to change variable types. However, the resulting variables would not run 
successfully in future procs, suggesting there is an issue with the method. 
I'm not sure how to fix it but spent significant time researching it.
Will be using Allen's code to attempt to fix it.*/
data train2; set train; 
life_sq2 = input(life_sq, best12.);
build_year2 = input(build_year, best12.);
material2 = input(material, best12.);
state2 = input(state, best12.);
product_type2 = input(product_type, best12.);
ecology2 = input(ecology, best12.);
run;

/*Allen's clean up code*/
...
/* Exporting cleaned up dataset due to SAS disconnecting too often */
proc export data=train4 
outfile="/home/mtrevathan0/APProject1train4.csv" 
dbms=csv
replace;
run;

/* For running everytime SAS disconnects */
proc import datafile="/home/mtrevathan0/APProject1train4.csv"
          dbms=dlm out=train4 replace;
     delimiter=',';
     getnames=yes;
     missing=0;
run;
...
/*Running scatterplots to see if there is a trend. I ran quite a few of them until I saw that Borock
had produced plots and uploaded them to Github. They still ran very slowly and took a large amoung of time
and frustration*/

ods graphics on;
proc sgscatter data=train4;
matrix price_doc full_sq area_m raion_popul;
run;
ods graphics off;

proc sgscatter data=train4;
matrix price_doc indust_part children_preschool preschool_quota children_school school_quota shopping_centers_raion office_raion full_all;
run;

proc sgscatter data=train4;
matrix price_doc male_f female_f young_all young_male young_female work_all work_male work_female ekder_all ekder_male ekder_female;
run;

proc sgscatter data=train4;
matrix price_doc build_count_block build_count_wood build_count_frame build_count_brick build_count_monolith build_count_panel build_count_foam build_count_slag build_count_mix;
run;

/*Running analysis of Brock's Model, with only numeric variables
Separated out due to SAS running times*/
proc sgscatter data=train4;
matrix price_doc full_sq area_m green_zone_part indust_part;
run;

proc sgscatter data=train4;
matrix price_doc children_preschool children_school male_f female_f;
run;

proc sgscatter data=train4;
matrix price_doc young_male work_male work_female ekder_male ekder_female;
run;

/*copied Allen's models to see results and see which variables are worth
following up on */
